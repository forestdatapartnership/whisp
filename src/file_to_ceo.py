    ...
    data = {
        ...
        "surveyQuestions": {
            "0": {
                ...
                "answers": {
                    "0": {"answer": "Yes", "color": "#5eb273", "hide": False},
                    "1": {"answer": "No", "color": "#d99c2e", "hide": False},
                },
                "parentQuestionId": -1,
                "parentAnswerIds": [],
                "dataType": "text",
...
def publish_project(project_id, cookie):
    ...
    try:
        response = requests.post(
            f"https://app.collect.earth/publish-project?projectId={project_id}&clearSaved=true",
            headers=headers,
        )
        response.raise_for_status()  # Raises an HTTPError if the HTTP request returned an unsuccessful status code
    except requests.RequestException as e:
...
def merge_and_reformat_to_gdf(roi, df, plot_id_column):
    ...
    df = pd.merge(
        geemap.ee_to_gdf(roi), df, left_on="Geo_id", right_on="Geo_id", how="inner"
    )
...
def gdf_to_shapefile_w_ceo_url(gdf, out_shapefile):
    ...
    gdf.rename(
        columns=lambda x: x[:10], inplace=True
    )  # Truncate column names for shapefile export
    ...
    # get collect earth online (ceo) url for viewing plots and stats (function from 'src.file_to_ceo')
